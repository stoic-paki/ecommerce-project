...
//     let confirm2 = confirm(`are you sure you want to continue?`)
//     if(confirm2){
//         res(`the message was confirmed`)
//     }
//     else{
//         rej(`the message was rejected`)
//     }
// })

// check.then((message_check)=>{
//     console.log(message_check)
// }).catch((message_check2) =>{
//     console.log(message_check2)
// })
...
let data = {
  ...
  function fetchProducts(productdata){
    let mappedData = productdata.map((meriProduct)=>{
        return `
            <div class="card">
    <img src="${meriProduct.thumbnail}" alt="img" class="img">
    <p class="title">${meriProduct.title}</p>
    <p class="price">${meriProduct.price}</p>
    <p class="desc">${meriProduct.description}</p>
    <div class="buttons-prod">
        <button class="add">Cart</button>
        <button class="remove" onclick="removeProduct(${meriProduct.id})">Remove</button>
    </div>
</div>
`
    })
  ...
  function createCards(){
    ...
    editCounter(data.products)
  }
  createCards()

function filterProd(product_category){
    let filtered_array = data.products.filter(
        (current_cat)=>current_cat.category === product_category 
    );

    console.log(filtered_array.length)
    editCounter(filtered_array)
    fetchProducts(filtered_array)
}

function editCounter(arr){
    counter.textContent=arr.length
}

function removeProduct(id){
    data.products = data.products.filter((myProduct)=> myProduct.id !== id)
    console.log("hello")
    createCards()
}

function searchBar(){
    let search_bar = document.querySelector("#search").value.toLowerCase()
    let searchFilter = data.products.filter((myProduct)=>{
        return myProduct.title.toLowerCase().startsWith(search_bar)
    })
    if(search_bar === ""){
        createCards()
    }
    editCounter(searchFilter)
